...
// -------------------------------------------------------------------
tf::MyGraph::MyGraph(const char* graph_file_name, const std::vector<std::string> & outputs, bool & success, int ninputs, int noutputs)
{
    ...
    */
    ...
    else // or last nodes with names containing provided strings
    {
        ...
        for (size_t n = 0; n < ng; ++n)
        {
            ...
            // else if (name.find("Identity") != std::string::npos) { basename = name; }
            // else { continue; }
    ...
    // TEMP
    std::cout << "fOutputNames:\n";
    for (auto val : fOutputNames) {
        std::cout << val << "\n";
    }
    std::cout << "fInputNames:\n";
    for (auto val : fInputNames) {
        std::cout << val << "\n";
    }
...
tf::MyGraph::~MyGraph()
{
    if ( ! fSession->Close().ok() ) {
      std::cout << "tf::MyGraph::dtor: " << "Close failed." << std::endl;
    }
    //fSession->Close();
    delete fSession;
}
...
std::vector< std::vector< std::vector<float> > > tf::MyGraph::Myrun(
	...
	long long int samples)
{
    ...
    // TEMP
    std::cout << "{" << samples << ", " << rows << ", " << cols << ", " << depth << "}\n";
...
std::vector< std::vector< std::vector< float > > > tf::MyGraph::Myrun(const std::vector< tensorflow::Tensor > & x)
{
    ...
    */
    ...
    for(auto tensor : outputs) {
        int num_dims = tensor.shape().dims();
        std::cout << "(";
