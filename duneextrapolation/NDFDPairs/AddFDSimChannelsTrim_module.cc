////////////////////////////////////////////////////////////////////////
// Class:       AddFDSimChannelsTrim
// Plugin Type: analyzer (Unknown Unknown)
// File:        AddFDSimChannelsTrim.cc
//
...
class extrapolation::AddFDSimChannelsTrim : public art::EDAnalyzer {
public:
  explicit AddFDSimChannelsTrim(fhicl::ParameterSet const& p);

  // Plugins should not be copied or assigned.
  AddFDSimChannelsTrim(AddFDSimChannelsTrim const&) = delete;
  AddFDSimChannelsTrim(AddFDSimChannelsTrim&&) = delete;
  AddFDSimChannelsTrim& operator=(AddFDSimChannelsTrim const&) = delete;
  AddFDSimChannelsTrim& operator=(AddFDSimChannelsTrim&&) = delete;
...
extrapolation::AddFDSimChannelsTrim::AddFDSimChannelsTrim(fhicl::ParameterSet const& p)
  : EDAnalyzer{p},
...
void extrapolation::AddFDSimChannelsTrim::analyze(art::Event const& e)
{
  // Get eventID
  ...
  for (const sim::SimChannel SC : *SCs) {
    ...
    const std::string groupPath = 
      "fd_simchannels_trim/" + std::to_string(eventID) + "/" + std::to_string(ch);

    fFile->createDataSet(groupPath, ionisations);
  }
}

void extrapolation::AddFDSimChannelsTrim::beginJob()
{
  fFile = new HighFive::File(fNDFDH5FileLoc, HighFive::File::ReadWrite);
}

DEFINE_ART_MODULE(extrapolation::AddFDSimChannelsTrim)
