...
void interactive::Session::beginJob()
{
  ...
  // Get ROPID signal types
  if (true) {
    using std::cout;

    unsigned int cID = 0;
    unsigned int tsetID = 0;

    readout::ROPID rID0(cID, tsetID, 0);
    readout::ROPID rID1(cID, tsetID, 1);
    readout::ROPID rID2(cID, tsetID, 2);
    readout::ROPID rID3(cID, tsetID, 3);

    if (fGeom->View(rID0) == geo::kU) { 
      cout << "ROP 0 is U\n";
    }
    else if (fGeom->View(rID0) == geo::kV) {
      cout << "ROP 0 is V\n";
    }
    else {
      cout << "ROP 0 is Z\n";
    }
    if (fGeom->View(rID1) == geo::kU) { 
      cout << "ROP 1 is U\n";
    }
    else if (fGeom->View(rID1) == geo::kV) {
      cout << "ROP 1 is V\n";
    }
    else {
      cout << "ROP 1 is Z\n";
    }
    if (fGeom->View(rID2) == geo::kU) { 
      cout << "ROP 2 is U\n";
    }
    else if (fGeom->View(rID2) == geo::kV) {
      cout << "ROP 2 is V\n";
    }
    else {
      cout << "ROP 2 is Z\n";
    }
    if (fGeom->View(rID3) == geo::kU) { 
      cout << "ROP 3 is U\n";
    }
    else if (fGeom->View(rID3) == geo::kV) {
      cout << "ROP 3 is V\n";
    }
    else {
      cout << "ROP 3 is Z\n";
