...
FDRECO_PAIR_OUTPUT="/pnfs/dune/scratch/users/awilkins/thesis_resp_trans/first_sample_150/ndfd_larndsim_3dpackets_infilled_fdreco_ndprojs_fdresppred_fdresppredreco_fdrecoinsidendlar"
...
lar -c standard_detsim_dune10kt_1x2x6_nooptdet.fcl -s LoadedFDDepsInsideNDOnly_g4.root -n -1
lar -c standard_reco_dune10kt_nu_1x2x6_nooptreco.fcl -s LoadedFDDepsInsideNDOnly_g4_detsimnoopt.root -n -1

# Add FD reco to H5 file
lar -c ./run_AddFDReco_InsideNDOnly.fcl -s LoadedFDDepsInsideNDOnly_g4_detsimnoopt_reconoopt.root -n -1
...
if [ "$SAVE_FDRECO_PAIR" = true ]; then
  ifdh cp ${input_name} ${FDRECO_PAIR_OUTPUT}/${input_name%.*}_fdrecoinsidendlar.h5
fi
